...
#threat exposure
ptsd <- read.csv("../abcd/release5.0/core/mental-health/mh_p_ksads_ptsd.csv")

#parent puberty
p_pds <-read.csv("../abcd/release5.0/core/physical-health/ph_p_pds.csv")

#youth BPS summary scores
bpm <- read.csv("../abcd/release5.0/core/mental-health/mh_y_bpm.csv")

#covariates and variables for sensitivity analyses
#site and family id 
site_fam <- read.csv("../abcd/release5.0/core/abcd-general/abcd_y_lt.csv")

#can extract "race_ethnicity" and income information ("demo_comb_income_v2") from this
demo <- read.csv("../abcd/release5.0/core/abcd-general/abcd_p_demo.csv")

#parent mental health data
parent_mh <- read.csv("../abcd/release5.0/core/mental-health/mh_p_asr.csv")

#height and weight measurements 
anthro <- read.csv("../abcd/release5.0/core/physical-health/ph_y_anthro.csv")
...
pdsTime <- ggplot(plotPdsData, aes(x=time, y=pds_avg, fill=sex)) +
  geom_rain(alpha = .5, adjust = 5) +
  theme_classic() +
  scale_fill_brewer(palette = 'Dark2') +
...
#remove impossible values - two participants with BMI values in thousands due to incorrect  height measurement?
bmi <- bmi %>% 
  filter(src_subject_id != c("NDAR_INV0P34UPZ9", "NDAR_INVZEKA1L7D"))

#also remove individuals with weight of 11lbs which is an improbable weight for a 9 year old
bmi <- bmi %>% 
  filter(src_subject_id != c("NDAR_INVK30HGMY4", "NDAR_INV4BAP6ERL"))
...
data <- cleanDf %>% 
  select(c("src_subject_id", "age_years", "BPM_T4", "trauma_T1", "trauma_cat", "trauma_bin", "sex", "time", "pds_avg", "pds_tot", "age_years", "rel_family_id", "site_id_l", "visit_type", "bmi_score", "race_ethnicity", "education_cat", "demo_comb_income_v2", "demo_roster_v2"))
head(data)
...
#select vars
youthPds <- y_pds %>% 
  select(src_subject_id, interview_date, interview_age, sex, eventname,
         pds_ht2_y, pds_skin2_y, pds_bdyhair_y, pds_m4_y, pds_m5_y, pds_f4_2_y, pds_f5_y)

#check number of I don't know responses across all time points for females

table(youthPds$pds_ht2_y) # N = 415
table(youthPds$pds_skin2_y) # N = 322
table(youthPds$pds_bdyhair_y) # N = 615
table(youthPds$pds_f4_2_y) # N = 553
table(youthPds$pds_f5_y) #N - 574
...
# #complete cases
# fYouthPdsCC <- complete.cases(fYouthPds)
# fYouthPdsCC <- sum(fYouthPdsCC) #N = 13834
# 
# #make CC df and get sample size per timepoint 
# fYouthPdsCC <- na.omit(fYouthPds)
#   tblPdsCC <- fYouthPdsCC %>% 
#               group_by(eventname) %>% 
#               count()
#   tblPdsCC
...
```{r, tidy variable names}

#create new time variable (same as eventname with shorter value names for plotting purposes)
youthPds <- youthPds %>% 
mutate(`time` = dplyr::recode(`eventname`,
                                `0_baseline_year_1_arm_1`="T0",
...
Note: need to think about how to handle missing data for predictor
...
Density plot of PDS per timepoint

```{r, plot}
pdsPlot <- ggplot(data=youthPds, aes(x=pds_avg, group=sex, fill=sex)) +
...
Raincloud plots - playing around with ggrain. 

See vignette: https://www.njudd.com/raincloud-ggrain/


```{r, ggrain}

pdsTime <- ggplot(youthPds, aes(x=time, y=pds_avg, fill=sex)) +
  geom_rain(alpha = .5, adjust = 5) +
  theme_classic() +
  scale_fill_brewer(palette = 'Dark2') +
  stat_summary(fun = mean, geom = "line", aes(group = sex, color = sex)) +
...
#this doesn't really work as there are too many data points! 

# p2 <- ggplot(youthPds, aes(x=eventname, y=pds_avg, fill=sex)) +
#   geom_rain(alpha = .5, rain.side = 'f2x2', id.long.var = "src_subject_id", adjust = 2) +
#   theme_classic() +
#   scale_fill_brewer(palette = 'Dark2')
# p2
# 
# p2 <- ggplot(youthPds, aes(eventname, pds_avg, fill = sex)) +
#   geom_rain(alpha = .5, rain.side = 'f', id.long.var = "src_subject_id",
...
We are going to use female adolescents only in our main analyses so let's explore the PDS data for females only further

```{r, PDS females}

femalePds <- youthPds %>% 
  filter(sex == "F") #obs = 18950
#remove male variables 
femalePds <- femalePds %>% 
...
pdsTable <- femalePds %>% 
  select(-c(src_subject_id, sex, interview_date, eventname)) %>% 
  tbl_summary(
    by = time,
    statistic = list(all_continuous() ~ c("{mean} ({sd})"),
                     all_categorical() ~ c("{n} / {N}")),
    digits = list(all_continuous() ~ c(2,2,0,0),
                  all_categorical() ~ c(0,0,2)),
    label = c(interview_age ~ "age",
              pds_ht2_y ~ "Height change", 
              pds_skin2_y ~  "Skin changes",
              pds_bdyhair_y ~ "Body hair changes",
              pds_f4_2_y ~ "Breast development",
              pds_f5_y ~ "Began menstruation",
              time ~ "time point",
           pds_avg ~ "PDS average score"),
    missing_text = "Missing",
    type = list (all_continuous() ~ "continuous"),
